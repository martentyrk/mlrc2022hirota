...
def get_parser():
    ...
    parser.add_argument("--remove_duplicates", default=True, type=bool)
...
def make_train_test_split(args, gender_task_mw_entries):
    print("Remove duplicates only relevant for caption models")
    ...
    print('#train : #test = ', len(d_train), len(d_test))
    print('Number of removed overlapping samples in test set:', (len_dtest_or -len(d_test)))
...
def train(model, iterator, optimizer, criterion, train_proc):
    ...
    for batch in iterator:
        ...
        #print("text_length", text_lengths.shape)
        #print(text_lengths)
...
def evaluate(model, iterator, criterion, batch_size, TEXT, args):
    ...
    if calc_mw_acc:
        ...
        male_acc, female_acc, male_score_avg, female_score_avg = None, None, None, None
